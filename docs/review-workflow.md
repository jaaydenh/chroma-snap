...
3. the worker reconciles the base-branch snapshot with the prior PR approval.

For the Milestone 6 MVP, reconciliation requires exact image SHA equality between the approved PR snapshot and the confirming base-branch snapshot. If they match, the worker promotes the base-branch capture as the new baseline and records a `baseline.promoted` audit event with the decision, approved build, reviewer, and confirming commit metadata. If they do not match, the worker leaves the existing baseline in place, keeps the check action-required, and records `baseline.promotion_mismatch`.

Approved deleted stories and modes are retired only after a base-branch run confirms the identity is still absent. The worker then deletes the baseline record and writes a `baseline.retired` audit event.

## Local MVP behavior

The current worker can seed base-branch baselines with `--seed-baselines`, classify PR-like manifests against those baselines, persist comparison reports, and use API-backed baseline/comparison/review stores for hosted worker handoff. The API records GitHub App webhooks, PR/base refs, strict Check Run state, review decisions, and audit events locally, and can publish updated GitHub Checks when a GitHub App publisher is configured. Review decisions require `write`, `maintain`, or `admin` repository permission through the local dev headers or an injected GitHub permission verifier.

Use `--reconcile-approved` on base-branch runs to promote or retire only previously approved PR changes that the base branch confirms. This mode reads prior comparison reports from `--comparison-file` and decisions/audit events from `--review-file`.
